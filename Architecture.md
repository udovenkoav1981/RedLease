...
место в общем счётчике. Поэтому параллельные Acquire в разных shards не могут
превысить лимит. Release, lazy expiry и on-demand cleanup возвращают место.
Acquire с эффективным TTL 0 ничего не хранит и место не занимает.

В каждом shard map дополнена min-heap, упорядоченной по ближайшему `deadline`.
Acquire, Renew и Release согласованно обновляют обе структуры; при продлении
lease операция Renew перемещает его элемент в heap.

Истёкшая запись удаляется лениво при следующей операции с тем же key. Если
...
Это server-side wire API для диагностики и служебных инструментов. Клиентская
библиотека не предоставляет публичный метод `Client.GetTTL`, не вызывает эту
операцию при старте и не использует её результат при сборе quorum.
...
Публичный `Lease.RemainingTTLms()` возвращает оставшуюся локальную validity в
...
Количество shards является параметром реализации и должно быть степенью двойки;
нулевое значение `server.Config.ShardCount` выбирает default 256. Благодаря
этому при создании `Server` один раз вычисляется `shardMask = shardCount-1`, а
индекс получается как `maphash(key) & shardMask` без целочисленного деления.
Предварительный keyed hash сохраняет равномерное распределение даже для
структурированных или намеренно подобранных значений `key`.
